...
parser.add_argument("-f16", "--use-float16", default=False, action="store_true",
                    help="Use 16-bit floating point model weights, instead of bfloat16. NOTE: f32 will be given priority.")
...
use_float16 = args.use_float16 and not args.use_float32
...
print("", "Loading model weights...", "_ @ {}".format(model_path), sep="\n", flush=True)
...
def main_loop():
    ...
    # --- Added code to extract directory and base filename for numeric navigation ---
    image_directory = os.path.dirname(image_path)
    current_filename = os.path.basename(image_path)
    # Attempt to extract the numeric part and extension
    try:
        name, ext = os.path.splitext(current_filename)
        current_image_number = int(name)
        image_extension = ext # Store extension for constructing next/prev filenames
        numeric_navigation_possible = True
    except ValueError:
        print(f"Error: Filename '{current_filename}' does not appear to be in a numeric format. Numeric navigation (n/p) disabled.")
        current_image_number = None # Indicate that numeric navigation is not possible
        image_extension = None
        numeric_navigation_possible = False
    # -------------------------------------------------------------------------------
    ...
    btn_next = btnbar.add_button("[n] Next Image", keypress="n")
    btn_prev = btnbar.add_button("[p] Prev Image", keypress="p")
    # -----------------------------------
    ...
    print("", "Displaying results",
          ...
          "  - Press n for next image, p for previous image (numeric filenames)", # Updated feedback
          "",
          sep="\n", flush=True)

    # --- Flag to indicate if a new image needs to be loaded ---
    reload_image = False
    # -------------------------------------------------------
    ...
    while True:
        ...
        # get next image
        key_pressed = keypress & 0xFF # Use keypress here

        # if key pressed is n: open next file and load depth maps
        if key_pressed == ord('n') and numeric_navigation_possible:
            found_next = False
            for i in range(1, 6): # Check next 5 numbers
                next_image_number = current_image_number + i
                next_filename = f"{next_image_number}{image_extension}"
                next_image_path = os.path.join(image_directory, next_filename)
                if os.path.exists(next_image_path):
                    image_path = next_image_path
                    reload_image = True
                    found_next = True
                    break # Found the next existing image, break the inner search loop
            if found_next:
                break # Break the display loop to reload image
            else:
                print(f"No next image found within the next 5 sequential numbers starting from {current_image_number + 1}{image_extension}.")
                # optional: produce sound if no depth maps found before or after
                # (Requires an additional library like 'playsound' or OS-specific calls)


        # else if key pressed is p: open previous file and load depth maps
        elif key_pressed == ord('p') and numeric_navigation_possible:
            found_prev = False
            for i in range(1, 6): # Check previous 5 numbers
                prev_image_number = current_image_number - i
                if prev_image_number < 0: # Prevent going to negative numbers
                    break
                prev_filename = f"{prev_image_number}{image_extension}"
                prev_image_path = os.path.join(image_directory, prev_filename)
                if os.path.exists(prev_image_path):
                    image_path = prev_image_path
                    reload_image = True
                    found_prev = True
                    break # Found the previous existing image, break the inner search loop
            if found_prev:
                break # Break the display loop to reload image
            else:
                print(f"No previous image found within the previous 5 sequential numbers starting from {current_image_number - 1}{image_extension}.")
                # optional: produce sound if no depth maps found before or after
                # (Requires an additional library like 'playsound' or OS-specific calls)

        elif (key_pressed == ord('n') or key_pressed == ord('p')) and not numeric_navigation_possible:
             print("Numeric navigation is not possible as the current filename is not numeric.")
    ...
    # --- Added check to reload image ---
    # This check is outside the inner while loop, so when the inner loop breaks
    # because reload_image is True, the outer main_loop checks this flag and
    # continues, effectively restarting the process with the new image_path.
    if reload_image:
        main_loop() # Recursively call main_loop with the new image_path
    # -----------------------------------

# Call the main loop to start the process
if __name__ == "__main__":
    main_loop()

# Clean up windows (this will be reached when the main_loop finally finishes without reloading)
cv2.destroyAllWindows()
